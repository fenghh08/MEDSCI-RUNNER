...
|---|---|---|---|
| `rooms/<CODE>` | Group Race host (creates), every racer (updates own player) | Every racer in that room, live via `onValue` | `status` (waiting/countdown/finished), `countdownAt`, `theme` + filters, `speed` (host's Game speed pick), `raceEndsAt`/`raceEndsReason`/`raceEndsByName` (`reason` is `finished`, `last-standing` or `host`; the first two are set once by a transaction, by whichever happens first — a racer finishing, or a racer becoming the sole survivor with everyone else done — starts the shared 45s catch-up window), `bombHellEndsAt`/`bombHellFromId`/`bombHellFromName` (set once by the Bomb Hell gift; every client except `bombHellFromId` clears non-bomb entities and spawns bombs on a tight timer until the deadline passes), `players/<playerId>` (name, glucose, stage, correct/incorrect counts, `shields`, `done`, `doneReason` incl. `timeup`, …), `winner`, `hostRole` (`'spectator'` when the host is a non-racing host — set only in that case; the host then has **no** `players/<id>` node, and a race with a single racer can resolve), `qstats/<itemId>::<qId>` (this race's per-question tally: `n` answered, `c` correct, `p0..p3` picks per authored option, `skip` timeouts — written by every racer via transaction, read by the host's results view) |
...
| `stats/<itemId>::<qId>` | The game, every time any player answers (a transaction that increments `correct` or `wrong`) | `developer-tool.html` → Data analysis tab (admin-only) | Aggregate per-question counts. `correct`/`wrong` cover every attempt (miss-rate table). `first/{n,c,picks,ab}` covers each player's **first** attempt only: `picks/p0..p3,skip` = how often each authored option was chosen (the option percentages in Question bank / Data analysis), and `ab/b00..b19` = answers bucketed by that player's own prior first-attempt accuracy on their device (5% bands, only once they have 10+ answers), from which the dev tool derives the discrimination index D (top 27% vs bottom 27% of responders) and the easy / poorly-written verdict. Still no per-player record anywhere — the ability tally lives only in the player's `localStorage`. Answers logged before this shape existed have no `first` node, so option percentages and D only fill in from new play. |
