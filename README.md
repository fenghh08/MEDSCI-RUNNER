...
| `developer-tool.html` | **The content tool.** Five tabs — Browse, Add, CSV import, Team queue, Maintainer — for adding, editing, or deleting items, questions, topics, courses and learning outcomes through forms with a live preview, bulk-importing from CSV, reviewing everything the team has queued, and (maintainer only) merging the queue into a complete, ready-to-replace `game-data.js`. Two more tabs, Contributors and Data analysis, appear once the maintainer PIN is unlocked. Every section has a small **i** button that explains how that part works under the hood. |
...
- **Team queue** — everything queued by anyone, live, with a quick count breakdown (items/questions/deletions/reports) and filters for content vs. reports/flags. A reported question has an **"Edit this question"** button that jumps straight into the Add form with that exact question already loaded, ready to fix and resubmit. Anyone can **🚩 Flag for removal** on a queue entry (a duplicate, a mistake) with a reason — no PIN needed, it just marks the row; actually removing it with **✕** still needs the maintainer PIN.
- **Maintainer** — PIN-locked: merge the queue into a new `game-data.js`, refresh the template, and a "is this folder up to date with GitHub?" check.
- **Contributors** and **Data analysis** — two more tabs that only appear once the Maintainer PIN is unlocked. Contributors groups everything by who added it (merged items/questions, whatever's still pending in the queue, and reports filed) so you can pick a name and see what they've contributed and their email, in one place. Data analysis is the correct/wrong miss-rate table per question (moved out of the Maintainer tab into its own).
...
Don't want to export to CSV first? The **"🧮 Paste into a table"** tab next to it is the same columns as an actual editable table — minus `description`/`mechanism`/`funfacts` (those describe an item, not a question; add/edit them from the item form instead, this table is questions only) — select a range in Google Sheets or Excel, copy, click a cell, and paste: it fills in every cell you copied at once (growing the table if the paste needs more rows), so there's no formatting-to-CSV step at all. Fix anything by hand, add/remove rows, then check & queue exactly like the CSV tab. The **Question ID** column is hidden by default (almost nobody needs to fill it in — it's auto-generated on merge if left blank); tick **"Show Question ID column"** above the table if you actually need it.
