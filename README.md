...
| `developer-tool.html` | **The content tool.** Opens on a **Start here** walkthrough of the contributor flow (template sheet → paste → team queue → admin merges), then tabs for bulk-importing by pasting a spreadsheet range into a table, adding lectures and learning outcomes, adding one thing at a time through forms with a live preview, browsing everything, reviewing the team queue and player reports, and (admin only) merging the queue into a complete, ready-to-replace `game-data.js`. Two more tabs, Contributors and Data analysis, appear once an admin unlocks. Every section has a small **i** button that explains how that part works under the hood. |
| `index.html`, `.nojekyll` | Only there for the website: `index.html` is the landing page at the GitHub Pages URL, with links to both the game and the developer tool, and the empty `.nojekyll` tells GitHub to serve the files as-is. Neither is needed for a local copy. |
...
directly -- it only becomes real content once an admin reviews and
merges it.

### 2. `developer-tool.html` — forms with a live preview

Open it next to `game-data.js`, type your name and email at the top (both are required before anything can be queued — it's how an admin credits and can reach you about what you add), and use the tabs on the left. Only an admin (PIN unlocked) can see whose submission is whose; everyone else sees "You" on their own queue entries and no name at all on anyone else's.

The tabs are grouped by what you're doing: **Contribute** (Start here, Bulk import, Lectures, Add content), **Browse** (Browse content, Question bank), **Review** (Team queue, Reports, Admin), and — once the Admin PIN is unlocked — **Admin tools** (Contributors, Data analysis).

- **Start here** — opens by default. The whole contributor flow in four steps, written for unit coordinators who work from the template sheet: get the sheet → fill in questions (and lectures/outcomes on its lectures tab) → paste into Bulk import or Lectures → it lands in the team queue for an admin to merge. Each step has a button straight to that tab, plus a plain "anyone can / admin only" table.
- **Browse** — the whole content tree (course → lecture → item → questions) with a search box, an "incomplete items" summary, and ✏️ buttons that jump straight into editing anything. Each course collapses/expands independently, so a big tree doesn't force you to scroll past courses you're not working in.
- **Lectures** — the lecture list for each course (L# · name · learning outcomes), for checking what a lecture actually covers without wading through items and questions. A pasteable table at the top lets anyone add or update lectures in bulk (same paste-a-spreadsheet-range interaction as Import CSV) — a lecture number that doesn't exist yet is created, one that already exists gets its name/outcomes updated. Single lectures can also be added via the "🧭 Learning outcomes" form under Add content.
- **Add** — one form each for an **item**, a **question**, a **topic**, a **course**, and **learning outcomes**. Every field renders in a preview panel styled like the real game, so what you see is what players will see. Existing courses/topics/items are picked from dropdowns, so a typo can't silently create a near-duplicate category. Creating a course (Admin-only) also registers it as a real course code (no random suffix on its id — just the name with spaces stripped, e.g. `AMED3003`) so it can immediately take lectures in the Lectures tab, not just topics/items. Editing is the same form with "I'm editing an existing item/question" ticked — it prefills every field with the real content, and submitting queues an update in place rather than a duplicate. Deleting (an item, one question from an item, or a whole course — the last from a 🗑 button next to the course in Browse content) works the same way — anyone can request it, no PIN needed, but it just asks why (a short reason) and queues the request; the actual deletion only happens once an admin reviews it and merges. Deleting a course removes it entirely (all its topics/items/questions) but can't touch `STAGES`/`RUNNABLE_THEMES` — those are template-only, so a Runner-mode course still needs a manual follow-up edit to fully retire it.
- **Bulk import** — see below. This is the main way in for anyone working from the sheet.
- **Team queue** — everything queued by anyone, live, with a quick count breakdown (items/questions/deletions/reports) split into two permanently separate lists: **📥 Content to merge** (items/questions/courses/topics/lecture outcomes/deletion requests — what an admin folds into `game-data.js`) and **⚠️ Reports from players** (flags on existing content sent from inside the game). A reported question has an **"Edit this question"** button that jumps straight into the Add form with that exact question already loaded, ready to fix and resubmit. Anyone can **🚩 Flag for removal** on a queue entry (a duplicate, a mistake) with a reason — no PIN needed, it just marks the row; actually removing it with **✕** still needs the admin PIN. Reports are never merged and never touched by "Remove the merged entries from the queue" — they only leave the queue when an admin removes them by hand.
- **Admin** — locked behind the PIN *and* an admin name list (your "Your name" field has to match one of Eason, Sara, Caitlin, Stephanie, Henry — case and spaces don't matter): merge the queue into a new `game-data.js`, refresh the template, and a "is this folder up to date with GitHub?" check.
- **Contributors** and **Data analysis** — two more tabs that only appear once the Admin PIN is unlocked. Contributors groups everything by who added it (merged items/questions, whatever's still pending in the queue, and reports filed) so you can pick a name and see what they've contributed and their email, in one place. Data analysis covers how players are actually doing: summary cards (answers recorded, overall accuracy, question coverage, most-answered question), accuracy broken down by course and by topic, and the per-question miss-rate table with a minimum-attempts filter so one-off lucky/unlucky answers don't skew what looks hardest. Every row expands to the full question (options, correct answer, explanation) with an edit link. A **Needs review** card auto-surfaces the questions most likely to be badly written or unclearly taught — a high miss rate over enough attempts, or a difficulty tag that doesn't match how it performs (tagged "easy" but mostly missed, or "hard" but almost never) — each with an edit link and a one-click **Flag for review** that files it into Reports for the team, the same way an in-game player report does.
...
### 3. Import CSV — for bulk work

One method: a real editable table, not a text box. Select a range in Google Sheets or Excel, copy, click a cell here, and paste — it fills in every cell you copied at once (growing the table if the paste needs more rows), so there's no formatting-to-CSV step at all. Course, topic and item are given per row; a blank Question ID creates a new question, a matching one updates that question in place, and **if the item doesn't exist yet it's created for you** — open it afterward to fill in description/mechanism/fun facts. Rows are checked as you type — a bad row is listed with its line number and, where relevant, a button straight to whatever's missing (add the course, the topic, or the lecture) rather than a dead end. The **Question ID** column is hidden by default (almost nobody needs to fill it in — it's auto-generated on merge if left blank); tick **"Show Question ID column"** above the table if you actually need it. **⬇️ Export all questions as CSV** gets you a starting point in the same shape, or use the linked template sheet.
...
## Merging into the real file (admin only)

The **Admin** tab of `developer-tool.html` is locked behind a 4-digit PIN (see `MAINTAINER_PIN` near the top of the `<script>` block — **change it from the default before handing this file to anyone**, it's plain text in the source and only meant to stop accidental clicks, not determined snooping).
...
Git is push-only for the admin here — contributors pull, they don't push directly. After merging a batch:
...
- **The admin PIN is a deterrent, not security.** It's readable in the page source by anyone who opens dev tools. It stops accidental destructive clicks, not a determined bypass.
